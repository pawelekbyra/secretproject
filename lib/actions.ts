...
// import { put, del } from '@vercel/blob'; // Removed
    ...
    try {
        ...
        if (avatarFile && avatarFile.size > 0 && avatarFile.name !== 'undefined') {
            ...
            if (session.user.avatar && session.user.avatar !== DEFAULT_AVATAR_URL) {
                try {
                   // Only attempt to delete if it is a local upload
                   if (session.user.avatar.startsWith('/uploads/')) {
                       const oldFilename = session.user.avatar.replace('/uploads/', '');
